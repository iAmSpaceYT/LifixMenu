...
void WriteVehicle(std::string name, Vehicle veh) {
	std::string iniPath = ".\\ShadowFolder\\ini files\\" + name + ".ini";
...
void WritePreset(std::string name, float id, float x, float y, float z, float rotx, float roty, float rotz, Hash model) {
	std::string iniPath = ".\\ShadowFolder\\ini files\\" + name + ".ini"; //change path
...
void Features::UpdateLoop() //bool loop
{
	...
	if ((GetTickCount() - ticker) >= Features::bandelay) {
		if (current_frame >= 197) { current_frame = 0; }
		else { current_frame++; }
		current_header = _strdup(("frame" + std::to_string(current_frame)).c_str());//frame5
		ticker = GetTickCount();
	}
	...
	vehicleflybool ? vehiclefly(true) : NULL;
...
void Features::paranormal(bool toggle)
{
	...
	{
		Menu::ScriptEventLog("~g~Press ~y~[ ~c~NUMPAD9 ~y~]!");
...
void Features::Superman(bool toggle)
{
	...
	notifyBottom("~g~Press ~y~[ ~c~Space ~y~]~g~, ~y~[ ~c~W ~y~]~g~, ~y~[ ~c~A ~y~]~g~, ~y~[ ~c~D ~y~]~g~, ~y~[ ~c~S ~y~]...");
...
bool spectate = false;
bool Flycarnigger3 = false;
float roll;
float yaw;
float pitch;
bool Features::vehicleflybool = false;
void Features::vehiclefly(bool toggle)
{
	if (Features::fcbool) {
...
void Features::Extremejump(bool toggle) {
	...
	Menu::ScriptEventLog("~g~Press ~y~[ ~c~Space ~y~]!");
}
...
void Features::bomber_mode(bool toggle) {
	if (PED::IS_PED_IN_ANY_HELI(PLAYER::PLAYER_PED_ID()) || PED::IS_PED_IN_ANY_PLANE(PLAYER::PLAYER_PED_ID())) {
		...
		GRAPHICS::DRAW_MARKER(6, bomb_coords.x, bomb_coords.y, bomb_coords.z, 0.f, 0.f, 0.f, 90.f, 0.f, 0.f, 15.f, 15.f, 15.f, rt, rrt, rrrt, 150, 0, 0, 0, 0, NULL, NULL, 0);
		...
		if (bomber_countdown) {
			if ((GetTickCount() - ticcker) >= 1500) {
				FIRE::ADD_EXPLOSION(bomb_coords.x, bomb_coords.y, bomb_coords.z, Features::Explosionfickk, Features::ficksizee, true, false, 0);
...
void Features::carboost(bool toggle) {
	...
	{
		Menu::ScriptEventLog("~g~Press ~y~[ ~c~E ~y~]!");
...
static void addOwnedExplosionToPed(Ped ped, float x, float y, float z, int explosionType, float damageScale, BOOL isAudible, BOOL isInvisible, float cameraShake)
{
...
void blame()
{
	...
	for (int i = 0; i < 32; ++i)
	{
		if (i != local && i != player)
		{
			...
			addOwnedExplosionToPed(ped, pos.x, pos.y, pos.z, 29, 20.f, true, false, 5.f);
		}
	}
...
if (Curvehicle != PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), false) && cargodmodebool) {
	if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), false))
	{
		Curvehicle = PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), false);
		ENTITY::SET_ENTITY_INVINCIBLE(Curvehicle, cargodmodebool);
		ENTITY::SET_ENTITY_PROOFS(Curvehicle, cargodmodebool, cargodmodebool, cargodmodebool, cargodmodebool, cargodmodebool, cargodmodebool, cargodmodebool, cargodmodebool);
		VEHICLE::SET_VEHICLE_DAMAGE(Curvehicle, 0.f, 0.f, 0.f, 0.f, 200.f, false);
		VEHICLE::SET_VEHICLE_ENVEFF_SCALE(Curvehicle, 0.f);
		VEHICLE::SET_DISABLE_VEHICLE_PETROL_TANK_DAMAGE(Curvehicle, cargodmodebool);
		VEHICLE::SET_DISABLE_VEHICLE_PETROL_TANK_FIRES(Curvehicle, cargodmodebool);
		VEHICLE::SET_VEHICLE_BODY_HEALTH(Curvehicle, 1000.f);
		VEHICLE::SET_VEHICLE_CAN_BE_VISIBLY_DAMAGED(Curvehicle, !cargodmodebool);
		VEHICLE::SET_VEHICLE_CAN_BREAK(Curvehicle, !cargodmodebool);
		VEHICLE::SET_VEHICLE_ENGINE_HEALTH(Curvehicle, 1000.f);
		VEHICLE::SET_VEHICLE_ENGINE_CAN_DEGRADE(Curvehicle, !cargodmodebool);
		VEHICLE::SET_VEHICLE_EXPLODES_ON_HIGH_EXPLOSION_DAMAGE(Curvehicle, !cargodmodebool);
		VEHICLE::SET_VEHICLE_PETROL_TANK_HEALTH(Curvehicle, 1000.f);
		VEHICLE::SET_VEHICLE_TYRES_CAN_BURST(Curvehicle, !cargodmodebool);
		VEHICLE::SET_VEHICLE_WHEELS_CAN_BREAK(Curvehicle, !cargodmodebool);
		VEHICLE::SET_VEHICLE_FIXED(Curvehicle);
		VEHICLE::SET_VEHICLE_DAMAGE(Curvehicle, 0.f, 0.f, 0.f, 0.f, 200.f, true);
		VEHICLE::SET_VEHICLE_DEFORMATION_FIXED(Curvehicle);
		VEHICLE::SET_VEHICLE_ENVEFF_SCALE(Curvehicle, 0.f);
		VEHICLE::SET_VEHICLE_DIRT_LEVEL(Curvehicle, 0.f);
		VEHICLE::SET_VEHICLE_BODY_HEALTH(Curvehicle, 1000.f);
		VEHICLE::SET_VEHICLE_ENGINE_HEALTH(Curvehicle, 1000.f);
		VEHICLE::SET_VEHICLE_PETROL_TANK_HEALTH(Curvehicle, 1000.f);
		if (VEHICLE::_IS_VEHICLE_DAMAGED(Curvehicle))
		{
			for (int i = 0; i < 10; i++)
			{
				try {
					VEHICLE::SET_VEHICLE_TYRE_FIXED(Curvehicle, i);
				}
				catch (...) {
					return;
				}
...
void Features::DepositToBank()
{
	...
	UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("Lifix", "Logo", 1, 8, "Shadow", "Recovery", 1, "", 9, 0);
...
void Features::maxvehicle()
{
	...
	VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(vehicle, "Lifixx");
}
...
void Features::ShootMiniGun(bool toggle)
{
	...
	Menu::ScriptEventLog("~g~Press ~y~[ ~c~NUMPAD_PLUS ~y~]!");
...
void Features::Speedometer(bool toggle) {
	...
	if (ENTITY::DOES_ENTITY_EXIST(VehicleEnt) && ENTITY::IS_ENTITY_A_VEHICLE(VehicleEnt))
	{
		...
		Menu::Drawing::Spriter("Lifix", "spedo1", speedoXfloatgrad, speedoYvaluegrad, widthgrad, heightgrad, 0.0f, 255, 255, 255, 255);
	}
}
