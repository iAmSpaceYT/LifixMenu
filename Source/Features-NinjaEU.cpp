...
//	fireammo ? Fireammos(true) : NULL;
	...
	/*bool Features::Stealthcash = false;
	void Features::stealthCash(bool toggle)
	{
		Vector3 pos =
			ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), FALSE);
		Any transID - INT_MAX:
		Hooking::create_ambient_pickup(0x1E9A99F8, &pos, 0, 1999,
			0x113FD533, 0, 0);
		Hooking::HK_BEGIN_SERVICE(transID, 1474183246,
			-1586170317, 1445302971, 9999999, 4);
		Hooking::create_ambient_pickup(0x1E9A99F8, &pos, 0, 1999,
			0x113FD533, 0, 0);*/
...
//void Spawn(std::string vehicle) 
{
	...
	if (STREAMING::IS_MODEL_VALID(model))
	{
		...
		if (spawnmaxed)
		{
			...
			VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(veh, "Exploit");
...
//bool Features::playerbunker = false;
//void Features::bunker(bool toggle)
//{
//	Vector3 Coords;
...
{
	Any idk = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(14999999, 15000000);
	UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&idk, 1474183246, -1586170317, 1445302971, amount, 1);
	UNK3::_NETWORK_SHOP_CHECKOUT_START(idk);
}

void Features::StealthDrop2(int amount) {
	Any idk = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(9999999, 10000000);
...
void Features::StealthLoop(bool toggle) {
	if ((timeGetTime() - Features::TimePD3) > 100)
	{
		Any idk = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(14999999, 15000000);
		UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&idk, 1474183246, -1586170317, 1445302971, 15000000, 1);
		UNK3::_NETWORK_SHOP_CHECKOUT_START(idk);
...
void Features::StealthLoop2(bool toggle) {
	if ((timeGetTime() - Features::TimePD4) > 100)
	{
		Any idk = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(14999999, 15000000);
...
void Features::StealthDropinte() {
	if ((timeGetTime() - Features::TimePD5) > 500)
	{
		if (amount > 0) {
			Any idk = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(14999999, 15000000);
			UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&idk, 1474183246, -1586170317, 1445302971, Features::amount, Features::banked ? 4 : 1);
			UNK3::_NETWORK_SHOP_CHECKOUT_START(idk);
		}
		else if (amount < 0) {
			Any izmon = 2147483647;
			if (UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&izmon, 1474183246, -1586170317, 1445302971, Features::amount, Features::banked ? 4 : 1)) {
...
void Features::cashdrop2(Player target)
{
	...
	if ((timeGetTime() - Features::TimePD2) > 550) // Time between drops
	{
...
void Features::carboost(bool toggle) {
	if (PLAYER::IS_PLAYER_PRESSING_HORN(PLAYER::PLAYER_ID()))
	{
		...
		{
			VEHICLE::SET_VEHICLE_FORWARD_SPEED(Veh, 150);
		}
	}
}
...
/*Ped Features::getLocalPlayerPed() {
	return Hooking::get_player_ped(Hooking::player_id());
}*/
...
void Features::OffRadar(bool toggle)
{
	if (Features::orbool)
	{
		ENTITY::SET_ENTITY_VISIBLE(Features::getLocalPlayerPed(), false, 0);
	}
	else
	{
		ENTITY::SET_ENTITY_VISIBLE(Features::getLocalPlayerPed(), true, 0);
	}
}*/
...
void Features::maxvehicle()
{
	...
	VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(vehicle, "Exploit");
}
...
namespace Features {
	...
	SCRIPT::TRIGGER_SCRIPT_EVENT(1, args, 30000, playerId);
	GAMEPLAY::CLEAR_BIT(&playerId, player);
	}
	void StealthDropende(Player player)
	{
	globalHandle(262145).At(5493).As<int>() = 50000 + *globalHandle(2097152).At(0).At(7546).At(1043).Get<int>();
	globalHandle(1591201).At(player).At(203).At(4).As<int>() = 50000;
	globalHandle(1591201).At(player).At(203).At(3).As<int>() = (*globalHandle(1591201).At(player).At(203).At(3).Get<int>()) + 50000;
	int playerId;
	GAMEPLAY::SET_BIT(&playerId, player);
	Any args[3] = { 0x00000127, player, 50000 };
	SCRIPT::TRIGGER_SCRIPT_EVENT(1, args, 3, playerId);
	GAMEPLAY::CLEAR_BIT(&playerId, player);
	}*/
	...
	void deletenearbyvehicles()
	{
		...
		for (int i = 0; i < VehFound; i++)
		{
			...
			{
				RequestControlOfEnt(vehs[OffsetID]);
				ENTITY::SET_ENTITY_COORDS_NO_OFFSET(vehs[OffsetID], 6400.f, 6400.f, 0.f, false, false, false);
			}
		}
		delete vehs;
	}
...
if ((timeGetTime() - Features::TimePD3) > 900)
{
	Any var0 = 0ul;
	int var3 = 312105838;
	int bankVal = 1;
...
void Features::StealthDropintee9() {
	if ((timeGetTime() - Features::TimePD3) > 900)
	{
		Any var0 = 0ul;
		int var3 = 312105838;
