...
void Menu::ScriptEventLog(const std::string& text)
{
	...
	UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2((char*)"Lifix", (char*)"Logo", true, 5, (char*)"Lifix", (char*)"lllIlllIllIIIIlll", 1.f, (char*)"___menu", 5, 0);
...
void FuckLuna()
{
	...
	Settings::optionRectSmooth.r = Settings::optionRect.r;
	Settings::optionRectSmooth.g = Settings::optionRect.g;
	Settings::optionRectSmooth.b = Settings::optionRect.b;
	static int i = 0;
	if (i % 1 == 0)
	{
		...
		{
			if (FadeEffect) {
				if (OR_OpacityID < Settings::optionRect.a) { OR_OpacityID++; }
				else { OR_OpacityID = Settings::optionRect.a; }
				if (OpacityIDFull < 255) { OpacityIDFull++; }
				else { OpacityIDFull = 255; FadeEffect = false; }
				Settings::line.a = OpacityIDFull;
				Settings::Endrectlol.a = OpacityIDFull;
				Settings::scroller.a = OpacityIDFull;
				Settings::count.a = OpacityIDFull;
				Settings::SelectedText.a = OpacityIDFull;
				Settings::ArrowAlpha = OpacityIDFull;
				Settings::titleEnd.a = OpacityIDFull;
				Settings::linego.a = OpacityIDFull;
				Settings::optionText.a = OpacityIDFull;
				Settings::OnThisDick.a = OpacityIDFull;
				Settings::NotOnThisDick.a = OpacityIDFull;
				Settings::subtitleText.a = OpacityIDFull;
				Settings::titleRect.a = OpacityIDFull;
				Settings::optionRectSmooth.a = OR_OpacityID; //HOW TF DID THIS EVEN WORK :Thonk:
				if (OpacityIDFull == 255)
					FadeEffect = false;
			}
		}
		else
		...
		if (Menu::Settings::Virustroll == true)
		{
			if (Menu::Settings::Authcheck == true)
			{
				...
				case mainmenu:
				{
					...
					Menu::MenuOption("Self Options", playermenu);//1
					Menu::MenuOption("Teleport", teleports);//5 
					Menu::MenuOption("Spawner", vehspawner);
					Menu::MenuOption("Vehicle", vehicle);//3
					Menu::MenuOption("Network", network);//4
					Menu::MenuOption("Weapon ", weapon);//2
					Menu::MenuOption("Recovery", recover);//6
					Menu::MenuOption("Protections", protection);//7
					...
					Menu::Info("Dive deep into the local options!", 1);
					Menu::Info("Yeet into places!", 2);
					Menu::Info("Spawn anything needed to you!", 3);
					Menu::Info("Make your vehicle fly!", 4);
					Menu::Info("Mess or be kind to your lobby!", 5);
					Menu::Info("Shoot very fast!", 6);
					Menu::Info("Get the $$$!", 7);
					Menu::Info("Protect against other modders!", 8);
					Menu::Info("Change the UI and look!", 9);
				}
				break;
				case creator:
				{
					Menu::Title("");
					Menu::Subtitle("CREATOR");
					Menu::MenuOption("Creator", creator);//2
					Menu::MenuOption("Spawn Pets", pet);
					Menu::MenuOption("Settings", settingsmenu);//9
				}
				break;
				...
				case network:
				{
					...
					Menu::MenuOption("Miscellaneous", mods);//8
...
#pragma endregion
				...
				case Wmods:
				{
					...
					Menu::Toggle("Gravity Gunn", Features::GravGun);
					...
					Menu::Toggle("Fire Amo", Features::fixfire, [] { Features::firefix(Features::fixfire); });
					Menu::Toggle("Explosive Amo", Features::Penis, [] { Features::penis(Features::Penis); });
...
#pragma region mapmode
				...
				case Remoteopt:
				{
					...
					if (Menu::Option("Crash Player")) {
						...
						notifyMap("Shadow: ~HUD_COLOUR_RED~ Player has been crashed successfully");
				...
				case recover:
				{
					...
					Menu::Break("~w~ Trigger Stealth 10Mil Method");

					if (Menu::Int("Stealth Amount", Menu::Settings::gayStealth, 1, 10000000))
					{
						if (IsKeyPressed(VK_NUMPAD5)) {
							Menu::Settings::gayStealth = NumberKeyboard();
						}
					}

					if (Menu::Int("Stealth Delay", Features::stealthdelay, 50, 1000))
					{
						if (IsKeyPressed(VK_NUMPAD5)) {
							Features::stealthdelay = NumberKeyboard();
						}
					}

					Menu::Toggle("Bank", Features::penisdunigger, [] { Features::weißernigger(Features::penisdunigger); });
				}
				break;
				...
				case unlockall: {
					...
					if (Menu::Option("~w~Reset Mental State"))
					{
						...
						UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_LESTER_DEATHWISH", "CHAR_LESTER_DEATHWISH", 1, 8, "~w~Shadow", "~w~Prime", 1, "UnlockAll", 9, 0);
						UI::_DRAW_NOTIFICATION(FALSE, FALSE);
					}
					...
					if (Menu::Option("~w~Unlock Office Money")) { // look i have globals off radar
						...
						UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_BUGSTARS", "CHAR_BUGSTARS", 1, 8, "Shadow", "Recovery", 1, "", 9, 0);
					...
					if (Menu::Option("Unlock All Tattoos")) {
						if (ENTITY::DOES_ENTITY_EXIST(PLAYER::PLAYER_PED_ID()))
						{
							...
							UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG_2("CHAR_LESTER_DEATHWISH", "CHAR_LESTER_DEATHWISH", 1, 8, "Shadow", "Prime", 1, "UnlockAll", 9, 0);
							UI::_DRAW_NOTIFICATION(FALSE, FALSE);
